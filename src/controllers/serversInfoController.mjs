import {queryGameServerInfo, queryGameServerPlayer} from "../steam-server-query/index.js";
...
async function fetchServerPlayersInfo(ip, port) {
    try {
        const serverInfo = await queryGameServerPlayer(`${ip}:${port}`, 1, 5000);
        return serializeBigInt(serverInfo);
    } catch (err) {
        return { error: `Failed to fetch server info for ${ip}:${port} - ${err.message}` };
    }
